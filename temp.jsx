...
export default function ProductionEntry() {
  ...
  useEffect(() => {
    if (operations.length > 0 && jobCardData && allJobCards.length > 0 && !nextOperationForm.next_operation_id) {
      ...
      if (nextOps.length > 0) {
        ...
        const isInhouse = mode.includes('IN_HOUSE') || mode.includes('INHOUSE') || !mode.includes('OUTSOURCE');
  ...
  const fetchAllData = async () => {
    try {
      ...
      // CRITICAL FIX: Base operations on Job Cards for this Work Order, enriched with global operation details
      const enrichedOperations = jobCards.map(jc => {
        ...
        return {
          ...
          execution_mode: jc.execution_mode || globalOp?.operation_type || 'IN_HOUSE',
          default_workstation: jc.machine_id || globalOp?.default_workstation || globalOp?.workstation || ''
        }
      })
      
      const sortedOperations = enrichedOperations.sort((a, b) => a.sequence - b.sequence)
      ...
      if (nextOpsList.length > 0) {
        ...
        console.log('Auto-filling Next Stage:', { nextOpId, nextWH, mode, isInhouse });
  ...
  const handleSubmitProduction = async () => {
    try {
      ...
      if (nextOperationForm.next_operation_id) {
        ...
        // Find next job card by operation ID/name AND sequence to be precise
        ...
        if (nextJobCard) {
          ...
          // Handle Outsourcing Automation
          if (nextOperationForm.outsource && nextOperationForm.auto_create_challans) {
            if (!nextOperationForm.vendor_id) {
              toast.addToast('Please select a vendor for outsourced operation', 'warning');
              setIsSubmitting(false);
              return;
            }

            const vendor = vendors.find(v => (v.supplier_id || v.id) === nextOperationForm.vendor_id);
            const vendorName = vendor?.name || vendor?.supplier_name;

            // 1. Create Outward Challan
            const outChallan = await productionService.createOutwardChallan({
              job_card_id: nextJobCard.job_card_id,
              vendor_id: nextOperationForm.vendor_id,
              vendor_name: vendorName,
              dispatch_quantity: transferableQty,
              notes: `Auto-generated from ${jobCardId} transfer`,
              items: [{
                item_code: jobCardData.item_code,
                qty: transferableQty,
                uom: jobCardData.uom
              }]
            });

            // 2. Create Inward Challan (as requested: "create both inward and outward challans")
            // This will effectively receive the items and make them ready for the next operation
            if (outChallan.success || outChallan.data) {
              const outChallanId = outChallan.data?.id || outChallan.id;
              await productionService.createInwardChallan({
                job_card_id: nextJobCard.job_card_id,
                outward_challan_id: outChallanId,
                vendor_id: nextOperationForm.vendor_id,
                vendor_name: vendorName,
                quantity_received: transferableQty,
                quantity_accepted: transferableQty,
                quantity_rejected: 0,
                notes: `Auto-received from ${jobCardId} transfer`
              });
            }
          }
        }
            ...
            <div className="col-span-12 space-y-8 pb-20">
              ...
              <div id="next-operation" className="bg-white rounded  border border-slate-100 p-4 scroll-mt-6">
                          ...
                          <SearchableSelect 
                            ...
                            onChange={(val) => {
                              ...
                              setNextOperationForm({ 
                                ...
                                inhouse: String(op?.execution_mode || op?.operation_type || '').toUpperCase().includes('IN_HOUSE') || 
                                        String(op?.execution_mode || op?.operation_type || '').toUpperCase().includes('INHOUSE') ||
                                        !String(op?.execution_mode || op?.operation_type || '').toUpperCase().includes('OUTSOURCE'),
                                outsource: String(op?.execution_mode || op?.operation_type || '').toUpperCase().includes('OUTSOURCE')
                              });
                            }} 
                            options={nextOps.map(op => ({ value: op.operation_id || op.id || op.operation_name || op.name, label: op.operation_name || op.name }))} 
                      ...
                      <div className="col-span-12 md:col-span-3">
                        <FieldWrapper label="Execution Mode">
                          <div className="flex gap-4 ">
                            <button onClick={() => setNextOperationForm({ ...nextOperationForm, inhouse: true, outsource: false })} className={`flex items-center gap-2 p-2  rounded border transition-all ${nextOperationForm.inhouse ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-slate-100 text-slate-400'}`}><span className="text-xs ">In-house</span></button>
                            <button onClick={() => setNextOperationForm({ ...nextOperationForm, outsource: true, inhouse: false })} className={`flex items-center gap-2 p-2  rounded border transition-all ${nextOperationForm.outsource ? 'bg-amber-50 border-amber-500 text-amber-700' : 'bg-white border-slate-100 text-slate-400'}`}><span className="text-xs ">Outsource</span></button>
                          </div>
                        </FieldWrapper>
                      </div>

                      {nextOperationForm.outsource && (
                        <>
                          <div className="col-span-12 md:col-span-3">
                            <FieldWrapper label="Vendor" required>
                              <SearchableSelect
                                value={nextOperationForm.vendor_id}
                                onChange={(val) => setNextOperationForm({ ...nextOperationForm, vendor_id: val })}
                                options={vendors.map(v => ({ value: v.supplier_id || v.id, label: v.name || v.supplier_name }))}
                                placeholder="Select Vendor"
                              />
                            </FieldWrapper>
                          </div>
                          <div className="col-span-12 md:col-span-3">
                            <FieldWrapper label="Automate Outsourcing">
                              <div className="flex items-center gap-3 mt-3">
                                <label className="relative inline-flex items-center cursor-pointer">
                                  <input 
                                    type="checkbox" 
                                    className="sr-only peer"
                                    checked={nextOperationForm.auto_create_challans}
                                    onChange={(e) => setNextOperationForm({ ...nextOperationForm, auto_create_challans: e.target.checked })}
                                  />
                                  <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                </label>
                                <span className="text-xs text-slate-500">Auto Create Outward/Inward Challans</span>
                              </div>
                            </FieldWrapper>
                          </div>
                        </>
                ...
                <div className="p-2 border-t border-slate-100 flex flex-col items-end gap-2">
                  <div className="text-[10px] text-slate-400 flex gap-3 px-2">
                    <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Accepted: {totalAcceptedQty.toFixed(0)}</span>
                    <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-indigo-400" /> Transferred: {transferredQty.toFixed(0)}</span>
                    <span className={`flex items-center gap-1 font-bold ${transferableQty > 0 ? 'text-indigo-600' : 'text-slate-400'}`}>
                  ...
                  <button
                    onClick={handleSubmitProduction}
                    disabled={isSubmitting || (!canTransfer && !isOperationFinished) || (hasPendingApproval && totalAcceptedQty === 0) || (!isOperationFinished && !nextOperationForm.next_operation_id)}
                    className={`flex items-center gap-3 p-2 rounded  transition-all shadow-lg active:scale-95 text-white ${
                      isOperationFinished 
                        ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200' 
                        : (transferableQty > 0 && (nextOperationForm.next_operation_id || isOperationFinished) ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200' : 'bg-slate-300 cursor-not-allowed shadow-none')
                    }`}
                  >
                    ...
                    {isOperationFinished ? 'Finalize & Dispatch' : (transferableQty > 0 ? `Transfer ${transferableQty.toFixed(0)} Units to ${operations.find(op => (op.operation_id || op.id || op.operation_name || op.name) === nextOperationForm.next_operation_id)?.name || 'Next Op'}` : 'Transfer Units')}
                  </button>
                  {!isOperationFinished && !nextOperationForm.next_operation_id && transferableQty > 0 && (
                    <p className="text-[10px] text-rose-500 animate-pulse">Please select Next Operation to enable transfer</p>
                  )}
                </div>
              </div>
