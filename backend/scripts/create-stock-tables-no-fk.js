...
async function deploySchema() {
  ...
  const tables = [
    ...
    },
    {
      ...
      sql: `CREATE TABLE IF NOT EXISTS stock_ledger (
        ...
        INDEX idx_item_warehouse_date (item_code, warehouse_id, transaction_date),
  ...
  try {
    console.log('📋 Creating stock module tables...\n')
    ...
    if (result && result.length > 0) {
      ...
      const stockTables = result.filter(r => {
        const name = Object.values(r)[0]
        return name.includes('stock') || name.includes('batch') || name.includes('material') || name.includes('reorder') || name.includes('warehouse')
      })
      console.log(`Stock-related tables: ${stockTables.length}`)
      stockTables.forEach(r => console.log(`  ✓ ${Object.values(r)[0]}`))
    }
