...
(async () => {
  try {
    ...
    const [existing] = await connection.execute(
      'SELECT id, bom_id, operation_name, operation_time FROM bom_operation ORDER BY bom_id, operation_name'
    );
    ...
    const [updated] = await connection.execute(
      'SELECT id, bom_id, operation_name, operation_time FROM bom_operation WHERE operation_time > 0 ORDER BY bom_id, operation_name'
    );
