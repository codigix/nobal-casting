    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Operational Intelligence" size="3xl">
      {loading ? (
        ...
        <div className="space-y-8 pb-8">
          {/* High-Level Overview */}
          <div className="bg-gray-900 rounded  p-8 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-8">
                <div className="space-y-3">
                  ...
                  <h2 className="text-3xl  tracking-tighter leading-none">{jobCard?.operation || 'Process Phase'}</h2>
                ...
                <div className="text-right">
                  ...
                  <div className="text-4xl  tracking-tighter text-indigo-400">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8 border-t border-white/10">
                <div className="">
                  ...
                  <p className="text-lg  tracking-tight">{jobCard?.planned_quantity} <span className="text-xs text-gray-500">Units</span></p>
                </div>
                <div className="">
                  <p className="text-xs   text-gray-500 ">Actual Output</p>
                  <p className="text-lg  tracking-tight text-emerald-400">{jobCard?.produced_quantity || 0} <span className="text-xs text-emerald-500/50">Units</span></p>
                </div>
                <div className="">
                  <p className="text-xs   text-gray-500 ">Assigned Unit</p>
                  <p className="text-lg  tracking-tight truncate">{jobCard?.machine_id || 'N/A'}</p>
                </div>
                <div className="">
                  <p className="text-xs   text-gray-500 ">Operator</p>
                  <p className="text-lg  tracking-tight truncate">{jobCard?.operator_id || 'Unassigned'}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50/50">
                      <th className="px-10 py-2  text-xs   text-gray-400  text-left">Timeline Sequence</th>
                      <th className="px-10 py-2  text-xs   text-gray-400  text-center">Operational Shift</th>
                      <th className="px-10 py-2  text-xs   text-gray-900  text-right">Gross Output</th>
                      <th className="px-10 py-2  text-xs   text-emerald-500  text-right">Validated Yield</th>
                      <th className="px-10 py-2  text-xs   text-rose-500  text-right">Rejection Rate</th>
                      <th className="px-10 py-2  text-xs   text-amber-500  text-right">Downtime Index</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {dailyLogs.map((log, idx) => (
                      <tr key={idx} className="hover:bg-indigo-50/30 transition-all duration-300 group">
                        ...
                        <td className="px-10 py-2  text-right  text-gray-900 text-sm tracking-tight">{log.produced}</td>
                        ...
                        <td className="px-10 py-2  text-right  text-rose-500 text-sm tracking-tight">{log.rejected}</td>
                        <td className="px-10 py-2  text-right  text-amber-500 text-sm tracking-tight">{log.downtime}m</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-gray-900 text-white">
                    <tr>
                      <td colSpan="2" className="px-10 py-6 text-xs    text-gray-500">Aggregate Yield Matrix</td>
                      <td className="px-10 py-6 text-right  text-white text-base tracking-tighter">{dailyLogs.reduce((sum, l) => sum + l.produced, 0)}</td>
                      <td className="px-10 py-6 text-right  text-emerald-400 text-base tracking-tighter">{dailyLogs.reduce((sum, l) => sum + l.accepted, 0)}</td>
                      <td className="px-10 py-6 text-right  text-rose-400 text-base tracking-tighter">{dailyLogs.reduce((sum, l) => sum + l.rejected, 0)}</td>
                      <td className="px-10 py-6 text-right  text-amber-400 text-base tracking-tighter">{dailyLogs.reduce((sum, l) => sum + l.downtime, 0)}m</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            <div className="bg-white rounded  border border-gray-100   overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50/50">
                      <th className="px-10 py-2  text-xs   text-gray-400  text-left">Phase Day</th>
                      <th className="px-10 py-2  text-xs   text-gray-400  text-left">Duration Window</th>
                      <th className="px-10 py-2  text-xs   text-gray-400  text-center">Shift</th>
                      <th className="px-10 py-2  text-xs   text-gray-900  text-right">Yield</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {timeLogs.map((log, idx) => (
                      <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                        <td className="px-10 py-2  text-sm  text-gray-900">D{log.day_number || '-'}</td>
                        <td className="px-10 py-2 ">
                        ...
                        <td className="px-10 py-2  text-right  text-gray-900 text-sm tracking-tight">{log.completed_qty}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-rose-50/30">
                      <th className="px-10 py-2  text-xs   text-rose-400  text-left">Incident</th>
                      <th className="px-10 py-2  text-xs   text-rose-400  text-left">Defect Reason</th>
                      <th className="px-10 py-2  text-xs   text-rose-600  text-right">Qty Loss</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-rose-50">
                    {rejections.map((rej, idx) => (
                      <tr key={idx} className="hover:bg-rose-50/50 transition-colors">
                        ...
                        <td className="px-10 py-2  text-xs  text-gray-600 italic">"{rej.rejection_reason || rej.reason || '-'}"</td>
                        <td className="px-10 py-2  text-right  text-rose-600 text-sm tracking-tight">{rej.rejected_qty}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
