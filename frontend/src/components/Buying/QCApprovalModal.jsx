    ...
    <Modal
      ...
      footer={
        <div className="flex gap-3 justify-end w-full px-4 py-2 bg-white border-t border-gray-100">
          ...
          {!showSendBackForm && (
            <div className="flex gap-3">
              <Button
                ...
                className="flex items-center gap-2 px-6   shadow-rose-100"
              >
              ...
              <Button
                ...
                loading={loading}
                disabled={!canApprove}
                className="flex items-center gap-2 px-8   shadow-emerald-100"
              >
          ...
          {showSendBackForm && (
            <div className="flex gap-3">
              <Button variant="ghost" onClick={() => setShowSendBackForm(false)} className=" text-gray-500 hover:text-gray-700">
              ...
              <Button
                ...
                loading={loading}
                className="flex items-center gap-2 px-8   shadow-rose-100"
              >
                Confirm Send Back
              </Button>
            </div>
      ...
      <div className="flex flex-col h-full bg-white">
        ...
        <div className="p-2  border-b border-gray-100 flex items-center justify-between sticky top-0 z-20 bg-white">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-emerald-600 rounded  flex items-center justify-center text-white  shadow-emerald-100">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl  text-gray-900 leading-tight">Quality Control Approval</h2>
              <p className="text-sm text-gray-500 font-medium mt-0.5 flex items-center gap-2">
                <ClipboardCheck className="w-3.5 h-3.5 text-emerald-500" />
                GRN <span className="text-indigo-600 ">#{grn.grn_no}</span> • Final verification before storage
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-2">
          ...
          {showSendBackForm ? (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="p-6 bg-rose-50 border border-rose-100 rounded  space-y-4">
                <div className="flex items-center gap-3 text-rose-800">
                  <div className="p-2 bg-rose-100 rounded ">
                    <History className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="">Revision Required</h3>
                    <p className="text-xs   tracking-wider opacity-70">Returning GRN to inspection phase</p>
                  </div>
                </div>
                
                <div className="space-y-2">
                  <label className="text-xs  text-rose-900   px-1">
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[120px] p-4 bg-white border border-rose-200 rounded  text-sm focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 outline-none transition-all resize-none shadow-inner"
                  />
                </div>
                
                <p className="text-xs text-rose-600 font-medium bg-white/50 p-3 rounded  flex items-start gap-2">
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded ">
                  <p className="text-[10px]  text-emerald-600  tracking-wider mb-1">Total Items</p>
                  <p className="text-xl  text-emerald-900">{grn.items?.length || 0}</p>
                </div>
                <div className="bg-emerald-50 border border-emerald-200 p-4 rounded ">
                  <p className="text-[10px]  text-emerald-700  tracking-wider mb-1">Accepted</p>
                  <p className="text-xl  text-emerald-900">{acceptedItems.length}</p>
                </div>
                <div className="bg-rose-50/50 border border-rose-100 p-4 rounded ">
                  <p className="text-[10px]  text-rose-600  tracking-wider mb-1">Rejected</p>
                  <p className="text-xl  text-rose-900">{rejectedItems.length}</p>
                </div>
                <div className="bg-indigo-50 border border-indigo-200 p-4 rounded ">
                  <p className="text-[10px]  text-indigo-700  tracking-wider mb-1">QC Pass Rate</p>
                  <div className="flex items-end gap-1.5">
                    <p className="text-xl  text-indigo-900">{itemsWithQCPass.length}/{acceptedItems.length}</p>
                    <p className="text-[10px]  text-indigo-500 mb-1 leading-none ">Items</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {!canApprove && acceptedItems.length > 0 && (
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded  flex items-center gap-3 animate-pulse">
                    <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0" />
                    <div>
                      <p className="text-sm  text-amber-900 leading-none mb-1">Action Required: QC Failures Detected</p>
                ...
                {rejectedItems.length > 0 && (
                  <div className="p-4 bg-rose-50 border border-rose-200 rounded  flex items-center gap-3">
                    <XCircle className="w-5 h-5 text-rose-600 shrink-0" />
                    <div>
                      <p className="text-sm  text-rose-900 leading-none mb-1">Inspection Results: Rejections Present</p>
                ...
                {canApprove && (
                  <div className="p-4 bg-emerald-50 border border-emerald-200 rounded  flex items-center gap-3">
                    <ShieldCheck className="w-5 h-5 text-emerald-600 shrink-0" />
                    <div>
                      <p className="text-sm  text-emerald-900 leading-none mb-1">Compliance Verified: Ready for Entry</p>
              ...
              {acceptedItems.length > 0 && (
                <div className="bg-white border border-gray-200 rounded  overflow-hidden  ">
                  <div className="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                    <h4 className="text-xs  text-gray-700   flex items-center gap-2">
                      <ClipboardCheck className="w-4 h-4 text-emerald-500" />
                      Standard Compliance Review
                    </h4>
                    <Badge variant="emerald" className="px-3 py-1 text-[10px]  ">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-white border-b border-gray-100">
                          <th className="p-2  text-left  text-gray-400   text-[10px]">Item Identification</th>
                          <th className="p-2  text-center  text-gray-400   text-[10px]">Recv. Qty</th>
                          <th className="p-2  text-center  text-gray-400   text-[10px]">Accp. Qty</th>
                          <th className="p-2  text-right  text-gray-400   text-[10px]">QC Protocol Status</th>
                        </tr>
                      </thead>
              ...
              {rejectedItems.length > 0 && (
                <div className="bg-rose-50/30 border border-rose-100 rounded  overflow-hidden  ">
                  <div className="px-5 py-3 bg-rose-50 border-b border-rose-100">
                    <h4 className="text-xs  text-rose-800  ">Flagged Rejections</h4>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-transparent border-b border-rose-100">
                          <th className="px-6 py-3 text-left  text-rose-400   text-[10px]">Item Code</th>
                          <th className="px-6 py-3 text-center  text-rose-400   text-[10px]">Qty</th>
                          <th className="px-6 py-3 text-left  text-rose-400   text-[10px]">Reason for Non-Compliance</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-rose-100/50">
                        {rejectedItems.map((item) => (
                          <tr key={item.id} className="hover:bg-rose-50/50 transition-colors">
                            <td className="p-2   text-rose-900">{item.item_code}</td>
                            <td className="p-2  text-center  text-rose-700">{item.received_qty}</td>
