...
const StatCard = ({ label, value, icon: Icon, trend, trendValue, bgColor, iconColor }) => (
  <div className="bg-white rounded p-3 border border-slate-200  hover: transition-all group relative overflow-hidden">
    <div className={`absolute -right-8 -bottom-8 w-32 h-32 rounded ${bgColor} opacity-10 group-hover:scale-110 transition-transform duration-700`} />
    <div className="relative z-0">
      <div className="flex items-center justify-between mb-4">
        ...
        {trend && (
          <div className={`flex items-center gap-1 text-xs  px-2.5 py-1 rounded ${trend === 'up' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
...
const DetailModal = ({ isOpen, item, onClose, detailedData, loading }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[100] p-4">
      <div className="bg-slate-50 rounded max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-white/20 shadow-2xl">
        <div className="bg-slate-900 p-6 flex items-center justify-between text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-40 bg-blue-600/10 rounded -mr-32 -mt-32 blur-3xl animate-pulse" />
          <div className="flex items-center gap-5 relative z-10">
            <div className="w-7 h-7 bg-gradient-to-br from-blue-500 to-indigo-600 rounded flex items-center justify-center text-2xl shadow-lg shadow-blue-500/30 ring-4 ring-white/10">
              <Building2 size={15} />
            </div>
            <div>
              <h2 className="text-xl text-white m-0 ">{profile.name || item.name}</h2>
              <div className="flex items-center gap-3 mt-1">
                <span className="text-slate-400 text-xs">ID: <span className="text-slate-200 ">{item.id}</span></span>
                <span className="w-1.5 h-1.5 rounded bg-slate-700" />
                <span className={`px-2 py-0.5 rounded text-xs    ${item.segment === 'Premium' ? 'bg-amber-500/20 text-amber-400 ring-1 ring-amber-500/30' : 'bg-blue-500/20 text-blue-400 ring-1 ring-blue-500/30'}`}>
                  {item.segment}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white p-2 rounded border border-slate-200  transition-shadow">
                    <div className="flex items-center gap-3 mb-3 text-blue-600">
                      <div className="p-2 bg-blue-50 rounded"><DollarSign size={15} /></div>
                      <p className="text-xs  text-slate-400   m-0">Lifetime Revenue</p>
                    </div>
                    <h4 className="text-xl  text-slate-900 m-0">₹{parseFloat(item.revenue).toLocaleString()}</h4>
                  </div>
                  <div className="bg-white p-2 rounded border border-slate-200  transition-shadow">
                    <div className="flex items-center gap-3 mb-3 text-purple-600">
                      <div className="p-2 bg-purple-50 rounded"><Package size={15} /></div>
                      <p className="text-xs  text-slate-400   m-0">Total Orders</p>
                    </div>
                    <h4 className="text-xl  text-slate-900 m-0">{item.orders} <span className="text-xs font-medium text-slate-400 ml-1 italic">Executions</span></h4>
                  </div>
                  <div className="bg-white p-2 rounded border border-slate-200  transition-shadow">
                    <div className="flex items-center gap-3 mb-3 text-emerald-600">
                      <div className="p-2 bg-emerald-50 rounded"><Zap size={15} /></div>
                      <p className="text-xs  text-slate-400   m-0">Avg Ticket Size</p>
                    </div>
                    <h4 className="text-xl  text-slate-900 m-0">₹{item.orders > 0 ? Math.round(item.revenue / item.orders).toLocaleString() : 0}</h4>
                  </div>
                </div>

                {/* Intelligence Layer: Supply Chain & OTD */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-900 p-2 rounded text-white relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700" />
                    <div className="relative z-10">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <Target size={15} className="text-blue-400" />
                          <h5 className="text-xs   text-white m-0">Delivery Fidelity</h5>
                        </div>
                        <span className="text-xs text-white text-slate-400">OTD RATE</span>
                      </div>
                      <div className="flex items-end gap-3 mb-4">
                        <h4 className="text-xl text-white  m-0">{otdRate}%</h4>
                        <div className={`flex items-center gap-1 text-xs  pb-1 ${otdRate >= 90 ? 'text-emerald-400' : 'text-amber-400'}`}>
                  ...
                  <div className="bg-white p-2 rounded border border-slate-200  relative overflow-hidden">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h5 className="text-xs  text-slate-900  text-white m-0">Supply Chain Velocity</h5>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="bg-slate-50 p-2 rounded border border-slate-100">
                        <p className="text-[8px]  text-slate-400  er mb-1">Awaiting</p>
                        <h4 className="text-xl  text-slate-900 m-0">{mrHealth.pending_mr}</h4>
                        <p className="text-[8px]  text-rose-500 mt-1">Pending MR</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded border border-slate-100">
                        <p className="text-[8px]  text-slate-400  er mb-1">In Pipeline</p>
                        <h4 className="text-xl  text-slate-900 m-0">{mrHealth.approved_mr}</h4>
                        <p className="text-[8px]  text-blue-500 mt-1">Approved</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded border border-slate-100">
                        <p className="text-[8px]  text-slate-400  er mb-1">Received</p>
                        <h4 className="text-xl  text-slate-900 m-0">{mrHealth.received_mr}</h4>
                        <p className="text-[8px]  text-emerald-500 mt-1">Ready</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-2 rounded border border-slate-200 ">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h4 className="text-sm  text-slate-900 m-0">Revenue & Volume Performance</h4>
                      <p className="text-xs text-slate-400 ">Bi-dimensional analysis of historical trends (6 Months)</p>
                    </div>
                    <div className="flex items-center gap-4 text-xs   ">
                      <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-blue-500" /> Revenue</div>
                      <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-purple-500" /> Orders</div>
                    </div>
                  </div>
                  <div className="h-72">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Top Items Bar Chart */}
                  <div className="bg-white p-2 rounded border border-slate-200 ">
                    <h4 className="text-sm  text-slate-900 mb-6 flex items-center gap-2">
                            ...
                            {statusDist.map((entry, index) => (
                              <div key={index} className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs  text-slate-600  er">{entry.name}</span>
                                </div>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-slate-900 rounded p-2 text-white shadow-xl">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pt-4 border-t border-white/10">
                      <div className="flex items-center justify-between">
                        <span className="text-xs  text-slate-500  ">Alliance Health</span>
                        <span className={`px-3 py-1 rounded text-xs   text-white ${profile.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                ...
                <div className="bg-white rounded p-2 border border-slate-200 ">
                  <h4 className="text-xs  text-slate-400   mb-6">Logistics Infrastructure</h4>
                  <div className="space-y-6">
                    <div className="relative pl-6 border-l-2 border-blue-500/30">
                      <div className="absolute top-0 left-[-7px] w-3 h-3 rounded bg-blue-500 ring-4 ring-blue-500/10" />
                      <p className="text-xs  text-slate-400  mb-2">Billing Nexus</p>
                    ...
                    <div className="relative pl-6 border-l-2 border-emerald-500/30">
                      <div className="absolute top-0 left-[-7px] w-3 h-3 rounded bg-emerald-500 ring-4 ring-emerald-500/10" />
                      <p className="text-xs  text-slate-400  mb-2">Shipping Terminal</p>
                ...
                <div className="bg-white rounded border border-slate-200  overflow-hidden">
                  <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <h4 className="text-xs  text-slate-400   m-0">Recent Activity</h4>
                    ...
                    {detailedData?.recentOrders?.length > 0 ? (
                      <div className="space-y-1">
                        {detailedData.recentOrders.map((order) => (
                          <div key={order.id} className="p-3 hover:bg-slate-50 rounded transition-colors group">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs  text-slate-900 group-hover:text-blue-600 transition-colors">{order.id}</span>
                              <span className="text-xs  text-slate-900">₹{parseFloat(order.amount).toLocaleString()}</span>
                            </div>
                            <div className="flex items-center justify-between">
...
export default function CustomerStatistics() {
  ...
  if (loading || !data) return (
    <div className="min-h-screen bg-slate-50 p-2 flex flex-col items-center justify-center p-3">
      <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded animate-spin mb-6" />
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 p-1.5 bg-white border border-slate-200 rounded text-xs  text-slate-700 hover:bg-slate-50 transition-all ">
            <Download size={14} /> Export
          </button>
          <button onClick={fetchStats} className="flex items-center gap-2 p-1.5 bg-blue-600 rounded text-xs  text-white hover:bg-blue-700 transition-all  shadow-blue-600/20">
            <Zap size={14} /> Refresh
          </button>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded p-4 mb-4 text-white flex flex-col md:flex-row items-center justify-between gap-6 border border-white/10 relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-64 h-40 bg-blue-600/10 rounded -mr-32 -mt-32 blur-3xl group-hover:bg-blue-600/20 transition-all duration-1000" />
        <div className="relative z-10 flex items-center gap-4">
          <div className="w-12 h-12 bg-white/10 rounded flex items-center justify-center text-blue-400 border border-white/10">
            <Zap size={24} className="animate-pulse" />
          </div>
          <div>
            <h3 className="text-sm  m-0 text-white ">Global Supply Chain Pulse</h3>
            <p className="text-xs text-slate-400  m-0  er">Real-time across {data.supplyChainHealth?.active_orders || 0} active manufacturing projects</p>
          </div>
        </div>

        <div className="relative z-10 flex flex-wrap items-center gap-8">
          <div className="text-center">
            <p className="text-xs text-slate-500   mb-1">Pending MRs</p>
            <div className="flex items-center gap-2">
              <span className="text-2xl  text-rose-400">{data.supplyChainHealth?.pending_mr || 0}</span>
              <span className="text-xs bg-rose-500/10 text-rose-500 px-1.5 rounded ">CRITICAL</span>
            </div>
          </div>
          <div className="w-px h-8 bg-white/10" />
          <div className="text-center">
            <p className="text-xs text-slate-500   mb-1">Approved MRs</p>
            <div className="flex items-center gap-2">
              <span className="text-2xl  text-blue-400">{data.supplyChainHealth?.approved_mr || 0}</span>
              <span className="text-xs bg-blue-500/10 text-blue-400 px-1.5 rounded ">ACTIVE</span>
            </div>
          </div>
          <div className="w-px h-8 bg-white/10" />
          <div className="text-center">
            <p className="text-xs text-slate-500   mb-1">Received MRs</p>
            <div className="flex items-center gap-2">
              <span className="text-2xl  text-emerald-400">{data.supplyChainHealth?.received_mr || 0}</span>
              <span className="text-xs bg-emerald-500/10 text-emerald-400 px-1.5 rounded ">READY</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-7 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-xs focus:outline-none focus:ring-1 focus:ring-blue-500/20 w-full md:w-48 transition-all"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <div className="flex gap-0.5 p-0.5 bg-slate-50 rounded border border-slate-200">
                <button onClick={() => setActiveTab('all')} className={`px-2 py-1 rounded text-xs transition-all font-medium ${activeTab === 'all' ? 'bg-white text-slate-900  ' : 'text-slate-400 hover:text-slate-600'}`}>ALL</button>
                <button onClick={() => setActiveTab('premium')} className={`px-2 py-1 rounded text-xs transition-all font-medium ${activeTab === 'premium' ? 'bg-white text-amber-600  ' : 'text-slate-400 hover:text-slate-600'}`}>PREMIUM</button>
                <button onClick={() => setActiveTab('regular')} className={`px-2 py-1 rounded text-xs transition-all font-medium ${activeTab === 'regular' ? 'bg-white text-blue-600  ' : 'text-slate-400 hover:text-slate-600'}`}>REGULAR</button>
              </div>
            </div>
            <p className="text-xs text-slate-400 font-medium">{filteredCustomers.length} Entities identified</p>
          </div>
            ...
            <table className="w-full text-left bg-white">
              <thead className="sticky top-0 z-10 bg-slate-50  ">
                <tr>
                  ...
                  <th className="p-2 text-xs  text-slate-400 ">Segment</th>
                ...
                {filteredCustomers.map((customer, idx) => (
                  <tr key={customer.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-2 text-center">
                      <span className="text-xs  text-slate-700 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">
                        {customer.orders}
                      </span>
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      <span className={`px-1.5 py-0.5 rounded text-[9px]  ${customer.segment === 'Premium' ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>
                    ...
                    <td className="p-2 text-right">
                      <button 
                        ...
                        className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 hover:bg-blue-600 hover:text-white rounded text-xs  text-slate-700 transition-all"
                      >
                        <Eye size={12} /> View
                      </button>
                    </td>
          ...
          <div className="space-y-1.5 mt-auto pt-2 border-t border-slate-50">
            {segmentationData.map((s, i) => (
              <div key={i} className="flex items-center justify-between p-1.5 bg-slate-50 rounded border border-slate-100">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded" style={{ backgroundColor: s.color }} />
        ...
        <div className="lg:col-span-2 bg-white p-2 rounded  border border-slate-200 ">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-3">
              <div className="flex items-center gap-1.5 text-xs  text-slate-500"><div className="w-2 h-2 rounded bg-amber-400" /> PREMIUM</div>
              <div className="flex items-center gap-1.5 text-xs  text-slate-500"><div className="w-2 h-2 rounded bg-blue-500" /> REGULAR</div>
            </div>
          </div>
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
